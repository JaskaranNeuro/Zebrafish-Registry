...
const RackGrid = ({ rack, onTankMove }) => {
  ...
  const effectiveRack = React.useMemo(() => ({ 
    ...rack, 
    row_configs: rack?.row_configs ? rack.row_configs : {} 
  }), [rack]);
  ...
  const getColumnCount = (rowIndex) => {
    ...
    const defaultCount = parseInt(effectiveRack.dimensions?.split('x')[1]) || 0;
  ...
  // Add this function to calculate cell width based on row
  const getCellWidth = (rowIndex) => {
    const customCount = effectiveRack.row_configs[rowIndex.toString()];
    const defaultCount = parseInt(effectiveRack.dimensions?.split('x')[1]) || 0;
    const defaultWidth = 120;
    if (customCount && parseInt(customCount) > 0) {
      const totalWidth = defaultWidth * defaultCount;
      return Math.max(totalWidth / parseInt(customCount), 60);
    }
    return defaultWidth;
  };

  // Build the grid with proper column counts per row
  const rows = parseInt(effectiveRack?.dimensions?.split('x')[0]) || 0;
              ...
              sx={{ 
                width: Math.max(
                  ...Array(rows).fill().map((_, idx) => {
                    const colCount = getColumnCount(idx);
                    const cellWidth = getCellWidth(idx);
                    return (colCount * cellWidth) + 150; // Increased padding to accommodate headers
                  })
                ),
                minWidth: '100%',
