...
const SuperAdminPage = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      try {
        ...
        const superAdminResponse = await axios.get('http://localhost:5000/api/check-super-admin', {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const fetchFacilities = async () => {
    try {
      ...
      const response = await axios.get(
        'http://localhost:5000/api/super-admin/facilities',
        {
          headers: {
  ...
  const handleExtend = async () => {
    try {
      ...
      const response = await axios.post(
        'http://localhost:5000/api/super-admin/subscription/extend',
        {
  ...
  const handleEnd = async () => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await axios.post(
        'http://localhost:5000/api/super-admin/subscription/end',
        {
          facility_id: selectedFacility.id
        },
        {
