...
const BreedingPlanList = ({ profile, onBack }) => {
  ...
  const fetchPlans = async (profileId) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/breeding/plans?profile_id=${profileId}`,
        {
          headers: {
  ...
  const handleDeletePlan = async (planId, event) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5000/api/breeding/plans/${planId}`, {
  ...
  const handleSavePlan = async (planData) => {
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/api/breeding/plans', dataWithProfile, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const handleUpdatePlan = async (planData) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(
        `http://localhost:5000/api/breeding/plans/${planToEdit.id}`,
        planData,
        {
          headers: { 'Authorization': `Bearer ${token}` }
        }
  ...
  const handleViewPlan = (plan) => {
    ...
    fetchTanks().then(() => {
      ...
      const token = localStorage.getItem('token');
      axios.get(
        `http://localhost:5000/api/breeding/plans/${plan.id}`, 
        { headers: { 'Authorization': `Bearer ${token}` } }
      ).then(response => {
  ...
  const handleCopyPlan = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.post(
        'http://localhost:5000/api/breeding/plans', 
        newPlanData,
        {
          headers: { 'Authorization': `Bearer ${token}` }
        }
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                        ...
                        <Button 
                          ...
                          onClick={() => {
                            ...
                            const token = localStorage.getItem('token');
                            axios.patch(
                              `http://localhost:5000/api/breeding/crosses/${cross.id}`, 
