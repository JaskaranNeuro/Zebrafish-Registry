...
const PaymentFormContent = ({ 
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5000/api/subscription/payment', {
      ...
      if (response.data.requires_action) {
        ...
        if (result.error) {
          ...
          try {
            ...
            const verifyResponse = await axios.post('http://localhost:5000/api/subscription/payment/verify', {
              payment_intent_id: response.data.payment_intent_id,
              plan: selectedPaymentPlan,
              period: selectedPaymentPeriod, // Use period instead of days
            }, {
...
const SubscriptionInfo = () => {
  ...
  const handleExtendSubscription = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.post('http://localhost:5000/api/subscription/extend', 
...
const handleEndSubscription = async () => {
  ...
  try {
    ...
    const response = await axios.post('http://localhost:5000/api/subscription/end', 
